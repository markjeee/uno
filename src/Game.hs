...
-- NOTE: For now, delegated to the playerIsOut for simplicty.
-- In effect, this means, only one round is supported.
...
takeAction action card gs
  ...
  | action == UseCard && not (cardCanPlay card gs) = error ("Card " ++ show card ++ " not match")
...
--playCurrentPlayer gs = (TakeFromDeck, whiteDummy)
...
useSimpleStrategy gs dcard hand
  | colorInHand (color dcard) hand = do
      (UseCard, getCardWithColor (color dcard) hand)
  | isNumberCard dcard && valueInHand (value dcard) hand = do
      (UseCard, getCardWithValue (value dcard) hand)
  | wildcardInHand hand = do
      (UseCard, colorizeWildcard Red (getWildcard hand))
